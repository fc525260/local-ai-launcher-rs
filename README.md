...
- 内置参数覆盖常用 `llama-server` 选项（线程、上下文、采样、RoPE/YaRN、推测解码、多模态、服务等），每个参数行和帮助说明都标注实际命令代码，如 `--gpu-layers (-ngl)`、`--temperature (-temp)`。
- “额外参数”区提供“参数速查”面板，列出其余所有 `llama-server` 参数及其说明，点击“添加”即生成对应控件（输入框/复选框/下拉）并加入额外参数区。
